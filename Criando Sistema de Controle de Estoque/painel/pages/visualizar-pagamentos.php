<div class="box-content">
	...
	<?php
		if(isset($_GET['email'])){
			...
			if(isset($_COOKIE['cliente_'.$cliente_id])){
				...
				$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_financeiro` WHERE id = $parcela_id");
				$sql->execute();
				$infoFinanceiro = $sql->fetch();
				$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_clientes` WHERE id = $cliente_id");
				$sql->execute();
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_financeiro` WHERE status = 0 ORDER BY vencimento ASC");
				$sql->execute();
				$pendentes = $sql->fetchAll();

				foreach ($pendentes as $key => $value) {
					$clienteNome = MySql::conectar()->prepare("SELECT `nome`,`id` FROM `tb_admin_clientes` WHERE id = $value[cliente_id]");
					$clienteNome->execute();
			...
			<tr <?php echo $style; ?>>
				...
				<td><a style="background: #00bfa5;" class="btn" href="<?php echo INCLUDE_PATH_PAINEL ?>visualizar-pagamentos?pago=<?php echo $value['id'] ?>"><i class="fa-solid fa-check"></i> Pago</a></td>
			</tr>
			<?php } ?>	
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_financeiro` WHERE status = 1 ORDER BY vencimento ASC");
				$sql->execute();
				$pendentes = $sql->fetchAll();
