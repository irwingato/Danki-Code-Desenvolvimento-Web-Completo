<?php
	$id = (int)$_GET['id'];
	$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_estoque` WHERE id = ?");
	...
	$pegaImagens = MySql::conectar()->prepare("SELECT * FROM `tb_admin_estoque_imagens` WHERE produto_id = $id");
	$pegaImagens->execute();
	$pegaImagens = $pegaImagens->fetchAll();	
?>

<div class="box-content">
	<h2><i class="fa fa-pencial"></i> Editando o Produto: <?php echo $id; ?></h2>	
	<div class="card-title"><i class="fa-solid fa-circle-info"></i> Informações do produto:</div>
	<?php
		if(isset($_GET['deletarImagem'])){
			$idImagem = $_GET['deletarImagem'];
			@unlink(BASE_DIR_PAINEL.'/uploads/'.$idImagem);
			MySql::conectar()->exec("DELETE FROM `tb_admin_estoque_imagens` WHERE imagem = '$idImagem'");
	...
	<form method="post" enctype="multipart/form-data">
				...
				if($sucesso){
					if($_FILES['imagem']['name'][0] != ''){
						...
						foreach ($imagens as $key => $value) {
							MySql::conectar()->exec("INSERT INTO `tb_admin_estoque_imagens` VALUES (null,$id,'$value')");
						}
					}

					$sql = MySql::conectar()->prepare("UPDATE `tb_admin_estoque` SET nome = ?,descricao = ?, altura = ?, largura = ?, comprimento = ?, peso = ?, quantidade = ? WHERE id = $id");
		...
		<div class="form-group">
			...
			<input type="number" name="largura" min="0" max="900" step="5" value="<?php echo $infoProduto['largura']; ?>">
		</div><!--form-group-->
