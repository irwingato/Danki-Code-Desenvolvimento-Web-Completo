<?php
	...
	if(isset($_POST['acao']) && $_POST['acao'] == 'inserir'){
		...
		$date = $_POST['data'];
		$sql = \MySql::conectar()->prepare("INSERT INTO `tb_admin_agenda` VALUES (null,?,?)");
		$sql->execute(array($data['tarefa'],$date));
		die(json_encode($data));
	}else if(isset($_POST['acao']) && $_POST['acao'] == 'puxar'){
		$data = $_POST['data'];
		$sql = \MySql::conectar()->prepare("SELECT * FROM `tb_admin_agenda` WHERE data = '$data' ORDER BY id DESC");
		$sql->execute();
		$info = $sql->fetchAll();
		$box = "";
