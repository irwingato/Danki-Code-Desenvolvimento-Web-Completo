...
<div class="box-content">
	<h2><i class="fa-solid fa-address-card" aria-hidden="true"></i> Produtos no estoque</h2>
	<div class="busca">
	...
	<?php
		...
		if(isset($_GET['deletar'])){
			...
			$id = (int)$_GET['deletar'];
			$imagens = MySql::conectar()->prepare("SELECT * FROM `tb_admin_estoque_imagens` WHERE produto_id = $id");
			$imagens->execute();
			...
			MySql::conectar()->exec("DELETE FROM `tb_admin_estoque_imagens` WHERE produto_id = $id");
			MySql::conectar()->exec("DELETE FROM `tb_admin_estoque` WHERE id = $id");
		...
		if(isset($_POST['atualizar'])){
			...
			if($quantidade < 0){
				...
				MySql::conectar()->exec("UPDATE `tb_admin_estoque` SET quantidade = $quantidade WHERE id = $produto_id");
				Painel::alert('sucesso','Você atualizou a quantidade do produto com ID: <b>'.$_POST['produto_id'].'</b>');
			}			
		}
		$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_estoque` WHERE quantidade = 0");
		$sql->execute();
		if($sql->rowCount() > 0){
			Painel::alert('atencao','Você está com produtos em falta! Clique <a href="'.INCLUDE_PATH_PAINEL.'visualizar-produtos?pendentes">aqui</a> para visualizá-los!');	
		}	
	...
	<div class="boxes">
		<?php
			...
			$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_estoque` $query $query2");
			...
			foreach ($produtos as $key => $value) {				
				$imagemSingle = MySql::conectar()->prepare("SELECT * FROM `tb_admin_estoque_imagens` WHERE produto_id = $value[id] LIMIT 1");
		...
		<div class="box-single-wraper">
				...
				<div style="width: 70%;float: left;border: 0;" class="box-single">				
					<div class="body-box">
						<p><b><i class="fa-solid fa-circle-info"></i>Nome do produto:</b><?php echo $value['nome'] ?></p>
						<p><b><i class="fa-solid fa-circle-info"></i>Descrição: </b><?php echo $value['descricao'] ?></p>
						<p><b><i class="fa-solid fa-circle-info"></i>Largura: </b><?php echo $value['largura'] ?>cm</p>
						<p><b><i class="fa-solid fa-circle-info"></i>Altura: </b><?php echo $value['altura'] ?> cm</p>
						<p><b><i class="fa-solid fa-circle-info"></i>Comprimento: </b><?php echo $value['comprimento'] ?> cm</p>
						<p><b><i class="fa-solid fa-circle-info"></i>Peso:</b> <?php echo $value['peso'] ?></p>
						<p><b><i class="fa-solid fa-circle-info"></i>Preço:</b> R$<?php echo Painel::convertMoney($value['preco']); ?></p>
...
<?php }else{?>
<div class="box-content">
	<h2><i class="fa-solid fa-address-card" aria-hidden="true"></i> <a href="<?php echo INCLUDE_PATH_PAINEL ?>visualizar-produtos"> Produtos no estoque</a> » Produtos em falta</h2>
	<?php		
