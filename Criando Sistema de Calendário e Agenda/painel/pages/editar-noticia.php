<?php
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$slide = Painel::select('tb_site_noticias', 'id = ?',array($id));
	}else{
			...
			if(isset($_POST['acao'])){
				...
				if($imagem['name'] != '') {
					...
					if(Painel::imagemValida($imagem)){
						...
						$arr = ['titulo'=>$nome,'data'=>date('Y-m-d'),'categoria_id'=>$_POST['categoria_id'],'conteudo'=>$conteudo,'capa'=>$imagem,'slug'=>$slug,'id'=>$id,'nome_tabela'=>'tb_site_noticias'];
						Painel::update($arr);
						$slide = Painel::select('tb_site_noticias','id = ?',array($id));
					...
					$imagem = $imagem_atual;
					$slug = Painel::generateSlug($nome);
					$arr = ['titulo'=>$nome,'categoria_id'=>$_POST['categoria_id'],'conteudo'=>$conteudo,'capa'=>$imagem,'slug'=>$slug,'id'=>$id,'nome_tabela'=>'tb_site_noticias'];
					Painel::update($arr);
					$slide = Painel::select('tb_site_noticias','id = ?',array($id));
