<?php
	...
	if(isset($_POST['acao']) && $_POST['acao'] == 'inserir_mensagem'){
		...
		$sql = MySql::conectar()->prepare("INSERT INTO `tb_admin_chat` VALUES (null,?,?)");
	...
	$_SESSION['lastIdChat'] = MySql::conectar()->lastInsertId();
		...
		$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_chat` WHERE id > $lastId");
		$sql->execute();
		$mensagens = $sql->fetchAll();
		$mensagens = array_reverse($mensagens);
		foreach ($mensagens as $key => $value) {
			$nomeUsuario = MySql::conectar()->prepare("SELECT nome FROM `tb_admin_usuarios` WHERE id = $value[user_id]");
			$nomeUsuario->execute();
