<?php
	$id = $par[1];
	$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_empreendimentos` WHERE id = ?");
...
<div class="box-content">
	...
	<div class="info-item">
		...
		<div class="row1">
			<div class="card-title"><i class="fa-solid fa-circle-info"></i> Imagem do empreendimento:</div>
			<img src="<?php echo INCLUDE_PATH_PAINEL ?>uploads/<?php echo $infoEmpreend['imagem'] ?>" />
		</div><!--row1-->

		<div class="row2">
				<div class="card-title"><i class="fa-solid fa-circle-info"></i> Informações do empreendimento:</div>
				<p><i class="fa fa-pen"></i> Nome do empreendimento: <?php echo $infoEmpreend['nome'] ?></p>
				<p><i class="fa fa-pen"></i> Tipo: <?php echo ucfirst($infoEmpreend['tipo']) ?></p>
		</div><!--row2-->
	...
	<div class="card-title"><i class="fa-solid fa-circle-info"></i> Cadastrar Imóvel:</div>
	...
	<?php
		if(isset($_POST['acao'])){
			...
			if($sucesso){
				...
				$sql = MySql::conectar()->prepare("INSERT INTO `tb_admin_imoveis` VALUES (null,?,?,?,?,0)");
				$sql->execute(array($empreendId,$nome,$area,$preco));
				$lastId = MySql::conectar()->lastInsertId();
				foreach ($imagens as $key => $value) {
					MySql::conectar()->exec("INSERT INTO `tb_admin_imagens_imoveis` VALUES (null,$lastId,'$value')");
				}
	...
	<div class="wraper-table">
		<table>
			...
			<?php
				$pegaImoveis = Painel::selectQuery('tb_admin_imoveis','id=?',array($id));
