<?php
	...
	if(isset($_POST['slug_empreendimento'])){
		...
		$infoEmpreendimento = \MySql::conectar()->prepare("SELECT * FROM `tb_admin_empreendimentos` WHERE slug = ?");
		$infoEmpreendimento->execute(array($slug_empreendimento));
		$infoEmpreendimento = $infoEmpreendimento->fetch();
		$sql = \MySql::conectar()->prepare("SELECT `tb_admin_imoveis`.* FROM `tb_admin_imoveis` WHERE (preco >= ? AND 
		...
		foreach ($imoveis as $key => $value) {
			$imagem = \MySql::conectar()->prepare("SELECT imagem FROM `tb_admin_imagens_imoveis` WHERE imovel_id = $value[id]");
			$imagem->execute();
		...
		$nome_imovel = $_POST['nome_imovel'];
		$sql = \MySql::conectar()->prepare("SELECT `tb_admin_imoveis`.* FROM `tb_admin_imoveis` WHERE `tb_admin_imoveis`.`preco` >= ? AND 
			`tb_admin_imoveis`.`preco` <= ? AND `tb_admin_imoveis`.`area` >= ? AND `tb_admin_imoveis`.`area` <= ? AND `tb_admin_imoveis`.`nome` LIKE ?");
