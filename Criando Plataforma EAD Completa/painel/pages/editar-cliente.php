<?php
	if(isset($_GET['id'])){
		$id = (int)$_GET['id'];
		$cliente = Painel::select('tb_admin_clientes', 'id = ?',array($id));
	}else{
...
<div class="box-content">
	<h2><i class="fa-solid fa-pen"></i> Editar Cliente</h2>

	<form class="ajax" atualizar method="post" action="<?php echo INCLUDE_PATH_PAINEL ?>ajax/forms.php" method="post" enctype="multipart/form-data">
	...
	<h2><i class="fa fa-pen"></i> Adicionar Pagamentos</h2>
		...
		if(isset($_POST['acao'])){
			...
			if(strtotime($vencimentoOriginal) < strtotime(date('Y-m-d'))){
				...
				for($i = 0; $i < $numero_parcelas; $i++){
					...
					$sql = MySql::conectar()->prepare("INSERT INTO `tb_admin_financeiro` VALUES (null,?,?,?,?,?)");
	...
	<?php
		if(isset($_GET['pago'])){
			$sql = MySql::conectar()->prepare("UPDATE `tb_admin_financeiro` SET status = 1 WHERE id = ?");
			$sql->execute(array($_GET['pago']));
			Painel::alert('sucesso','O pagamento foi quitado com sucesso!');
		}
	?>

	<h2><i class="fa-solid fa-money-check-dollar"></i> Pagamentos Pendentes</h2>
	...
	<table>
		...
		<?php
			$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_financeiro` WHERE status = 0 AND cliente_id = $id ORDER BY vencimento ASC");
			$sql->execute();
			$pendentes = $sql->fetchAll();

			foreach ($pendentes as $key => $value) {
				$clienteNome = MySql::conectar()->prepare("SELECT `nome` FROM `tb_admin_clientes`WHERE id = $value[cliente_id]");				
				$clienteNome->execute();
		...
		<tr <?php echo $style; ?>>
			...
			<td><a class="btn edit" href="<?php echo INCLUDE_PATH_PAINEL ?>"><i class="fa-solid fa-envelope"></i> E-mail</a></td>			
			<td><a style="background: #00bfa5;" class="btn" href="<?php echo INCLUDE_PATH_PAINEL ?>editar-cliente?id=<?php echo $id; ?>&pago=<?php echo $value['id'] ?>"><i class="fa-solid fa-check"></i> Pago</a></td>
		</tr>
		<?php } ?>	
	...
	<h2><i class="fa-solid fa-money-check-dollar"></i> Pagamentos Concluídos</h2>
	...
	<table>
		...
		<?php
			$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_financeiro` WHERE status = 1 AND cliente_id = $id ORDER BY vencimento ASC LIMIT 10");
			$sql->execute();
			$pendentes = $sql->fetchAll();
