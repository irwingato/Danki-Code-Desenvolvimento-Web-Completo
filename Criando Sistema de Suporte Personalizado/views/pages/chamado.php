...
<?php
	$puxaInteracoes = \MySql::conectar()->prepare("SELECT * FROM interacao_chamado WHERE id_chamado = ?");
	echo '<hr>';

	$puxaInteracoes->execute(array($token));
	$puxaInteracoes = $puxaInteracoes->fetchAll();
	foreach ($puxaInteracoes as $key => $value) {
		if($value['admin'] == 1){
...
<?php
	if(isset($_POST['responder_chamado'])){
		...
		echo '<script>alert("Sua resposta foi enviado com sucesso! Aguarde o admin responde-lo(a) :)")</script>';
	...
	if($sql->rowCount() == 0){
		...
		$info = $sql->fetchAll();
		if($info[0]['admin'] == -1){
			//A última interação foi feita por quem abriu o suporte. Nâo pode interagir até ter uma resposta.
			echo '<p>Aguarde até ter uma resposta do admin para continuar com o suporte!</p>';
		}else{
