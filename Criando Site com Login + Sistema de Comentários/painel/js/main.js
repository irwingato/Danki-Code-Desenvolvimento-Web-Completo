$(function(){
	...
	var targeSizeMenu = (windowSize <= 400) ? 200 : 300;
	...
	$('.menu-btn').click(function(){
		if(open){
			...
			$('.menu').animate({'width':targeSizeMenu+'px','padding':'10px'},function(){
				open = true;
			});
			$('.content,header').css('width','calc(100% - 250px)');
			$('.content,header').animate({'left':targeSizeMenu+'px'},function(){
				open = true;
			});
		}
	})
	...
	$(window).resize(function(){
		...
		targeSizeMenu = (windowSize <= 400) ? 200 : 250;
		if(windowSize <= 768){
