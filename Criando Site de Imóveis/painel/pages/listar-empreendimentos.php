<div class="box-content">
	<h2><i class="fa-solid fa-address-card" aria-hidden="true"></i> Empreendimentos</h2>	
	<div class="busca">
	...
	<?php
		...
		if(isset($_GET['deletar'])){
			...
			$id = (int)$_GET['deletar'];
			$imagens = MySql::conectar()->prepare("SELECT `imagem` FROM `tb_admin_empreendimentos` WHERE id = $id");
			$imagens->execute();
			$imagens = $imagens->fetch();
			@unlink(BASE_DIR_PAINEL.'/uploads/'.$imagens['imagem']);

			$imoveis = MySql::conectar()->prepare("SELECT * FROM `tb_admin_imoveis` WHERE empreend_id = $id");
			$imoveis->execute();
			$imoveis = $imoveis->fetchAll();
			foreach ($variable as $key => $value) {
				$imagens = MySql::conectar()->prepare("SELECT `imagem` FROM `tb_admin_imagens_imoveis` WHERE imovel_id = $value[id]");
				$imagens->execute();
				$imagens = $imagens->fetchAll();
				foreach ($imagens as $key2 => $value2) {
					@unlink(BASE_DIR_PAINEL.'/uploads/'.$value2['imagem']);
					MySql::conectar()->exec("DELETE FROM `tb_admin_imagens_imoveis` WHERE id = ");	
				}
			}
			MySql::conectar()->exec("DELETE FROM `tb_admin_imoveis` WHERE empreend_id = $id");
			MySql::conectar()->exec("DELETE FROM `tb_admin_empreendimentos` WHERE id = $id");
	...
	<div class="boxes">
		<?php
			...
			$sql = MySql::conectar()->prepare("SELECT * FROM `tb_admin_empreendimentos` $query ORDER BY order_id ASC");
		...
		?>
		<div id="item-<?php echo $value['id']; ?>" class="box-single-wraper" style="padding-right: 10px 30px;">
			...
			<div style="width: 70%;float: left;border: 0;" class="box-single">
				<div class="body-box">
					<p><b><i class="fa fa-pencil"></i> Nome do Empreendimento:</b> <?php echo $value['nome'] ?></p>
